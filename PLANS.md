...
## 当前未完成计划
- 对 `消化系统/PBL作业/Case3-第一幕/Case3-第一幕.md` 做提交前最后精修，重点修复引用编号错位、课堂痕迹残留与关键文献格式。
  - 优先清除正文中的“课堂笔记/老师提示/课堂讨论”等提交版不应出现的来源痕迹。
  - 逐题核对正文 `[n]` 与本题 `④出处` 的第 `n` 条是否一一对应，尤其先修问题1-4，再覆盖问题5-10。
  - 按已核实的权威来源补正文献格式，重点补全 APASL 2025、EASL 2025、AASLD/IDSA 慢乙肝指导等条目。
  - 重新生成 `Case3-第一幕-美化版.html` 与 `Case3-第一幕-美化版.pdf`，并做残留词、结构层级和 `qpdf --check` 验证。
  - 同步更新 `PROGRESS.md` 并提交本轮最终精修结果。

...
## 当前未完成计划
- `循环系统/PBL作业/Case2-第三幕` Markdown 清洗与出处修订：
  1. 已先将原始 `Case2-第三幕.md` 单独提交为基线备份，后续清洗不覆盖原始历史。
  2. 参考第二幕成品格式，把第三幕整理为正式作业结构：`# Case 2 第三幕 作业`、基本信息、问题列表、关键词、逐题 `③学习内容 / ④出处 / ⑤实际应用`。
  3. 清除 AI 对话痕迹、草稿提示、正文直链、`utm_source`、Markdown 底部链接定义和“教材页码待核对/相关章节”之类不适合提交的表达。
  4. 保留正文中自然的第一人称病例推理语气，例如“我认为/我倾向于/我觉得”，本轮不主动改成过度客观的 AI 式语气。
  5. 将每题出处改为连续数字列表，正文引用改为对应 `[1]`、`[2]` 等编号；尽量把引用标在具体论断后，而不是只堆在段末。
  6. 核对《内科学》第10版教材页码；凡本地无法核对的其他教材或专著，不臆造页码，保留明确待用户补书的标记并在最终说明里列出。
  7. 复制或复用第二幕 HTML 构建脚本与样式，确认引用上标跳转、题内出处锚点、标题层级、外链收敛和 PDF 导出均正常。
  8. 完成后运行残留关键词、正文外链、引用编号闭环、HTML/PDF 生成和 `qpdf --check` 等验证，并更新 `PROGRESS.md`。

...
- 完成 `消化系统/PBL作业/Case3-第三幕` PBL 作业最终版整理与 PDF 导出。
  - 重要边界：不整篇重写第三幕正文，不改变原有论述风格和病例表达；只在原文基础上做逐点补丁。
  - 先按既有 Case2 / Case3 格式，最小化修正标题层级：`# 标题`、`## ①列出所有问题`、`## ②整幕关键词`、`## 问题N`、`### ③学习内容 / ④出处 / ⑤实际应用`。
  - 只清理明确影响交付的内容：脚注式链接定义、`utm_source/chatgpt` 污染、旧版教材条目、其他教材/专著条目、页码缺失或泛泛“相关章节”。
  - 教材/专著出处只保留《内科学》第10版；删除《药理学》《外科学》《消化内镜学》《Harrison》等其他教材/专著条目，但不因此重写正文段落。
  - 用根目录《内科学 （第10版）.pdf》逐条核对第三幕涉及的病毒性肝炎、肝硬化、消化道出血、输血、肝癌等页码和章节位置；只把核实后的教材页码补入出处。
...
- 新增 `消化系统/PBL作业/Case3-第三幕` 整幕思维导图，按 `6.5PBL.txt` 老师课堂原文和 Case2 第三幕导图修订经验收口。
  - 先提炼老师要求：导图是“对一种或一类疾病的概括、归纳、总结”，不是按时间顺序堆知识点，也不是把每一幕问题做成表格。
  - 中心主题应先写疾病主线，再把孙奶奶病例的具体诊断、出血事件、内镜治疗、抗病毒和长期随访作为病例证据。
  - 一级主题保持并列、短小、逻辑清楚，参考 Case2 正式版的 `病因与危险因素 / 发病机制 / 诊断 / 鉴别诊断 / 治疗与管理 / 预后与并发症`。
  - 第三幕治疗内容要归入疾病总图的治疗与长期管理分支，不单独做“第三幕”分支；三幕证据放到右上角病例证据卡。
  - 验收包括：HTML 基本结构检查、6 个一级主干检查、草稿词残留检查、老师要求关键词对应检查、PowerShell 本地静态读取检查；如环境允许，再用本地浏览器或简易 HTTP 验证页面可加载。

- 按“课程/材料类型”逐波次整理并提交 `大二下笔记` 当前工作区，并在收口后把仓库历史里的 `ppt/pptx` 也一并踢出后强推远端。
  - 2026-07-03 新要求：本轮先按“新增课堂资料”做最小影响面分批提交并 push 当前课程仓库；同时定位并同步处理用户提到的 `notes-cavas/notes-canvas` 仓库，把其待提交改动与部署链路一并收口。

...
## 当前未完成计划
- 修正 `消化系统/PBL作业/Case3-第一幕/Case3-第一幕.md`，按 `Case2` 已固定的 PBL Markdown 骨架完成清洗、补引文与转换验证。
  - 先核对 `Case2-第一幕/第二幕/第三幕` 的最终标题层级、题内 `③学习内容 / ④出处 / ⑤实际应用` 结构，以及 `build_html.py` 对 Markdown 的格式要求。
  - 先对当前 `Case3-第一幕.md` 做一笔“原样基线”提交，保证后续清洗、补页码和脚本适配都有清晰对照。
  - 清理正文中的 AI 连接语、口语化制作提示、脚注直链、`utm_source`、错误教材写法和不规范标题层级，保留病例正文与正确引用骨架。
  - 统一把教材出处收敛为《内科学》第10版，并尽量补到具体章节与页码；若某条当前证据不足以落到精确页码，则宁可删去，不保留“相关章节”式模糊写法。
  - 按 `Case2` 的做法尽可能给正文补 `[1]` 形式的题内引用编号，并修正每题出处列表的编号闭环与链接格式。
  - 迁移或复用可用的 `build_html.py` / `style.css` 到当前目录，跑通 HTML/PDF 转换并完成结构、引用残留、脚本与 PDF 完整性验证。
  - 同步更新 `PROGRESS.md`，记录本轮基线提交、清洗范围、教材页码取舍和转换验证结论。

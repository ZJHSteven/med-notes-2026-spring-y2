...
## 当前未完成计划
- `循环系统/PBL作业/Case2-第一幕` 汇报整合 PPT 修订：
  1. 核对上一版整合稿和全部源 PPT 的页面比例、页数与可打开状态。
  2. 将最终整合稿重建为 16:9 宽屏比例，避免放映时变成 4:3。
  3. 用 PowerPoint 原生复制/插入方式合并可打开的同学 PPT，尽量保留同学原稿页面，不重新改写版式。
  4. 将新收到的 `case2第一幕.pptx` 插入到“问题1：心梗病因、诱因”对应位置。
  5. 重新验证 PPTX 可打开、页数、比例、导出预览、媒体完整性、草稿词残留和对象越界情况。

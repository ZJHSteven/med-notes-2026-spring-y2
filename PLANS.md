...
## 当前未完成计划
- `循环系统/PBL作业/Case2-第二幕` Markdown 清洗与 HTML/PDF 构建：
  1. 以第一幕 `build_html.py` 与 `style.css` 为基准，迁移到第二幕目录，并补强正文引用上标跳转、出处锚点、`###/####` 标题渲染能力。
  2. 清洗 `Case2-第二幕.md`：删除 AI 生成痕迹、正文直链、`utm_source`、重复来源提示和“后续核对”类草稿话术；统一为 `## 问题` + `### ③/④/⑤` + `####` 内部小标题结构。
  3. 将每题 `④出处` 统一改为第一幕式数字列表；正文只保留 `[1]` 形式引用，链接只放在出处条目里。
  4. 教材出处优先复用第一幕已核准的《内科学》第10版章节与页码；暂时无法由现有材料核准的具体章节必须留下醒目标记，不能写成泛泛“相关章节”。
  5. 重新生成第二幕美化版 HTML/PDF，并用脚本检查残留噪声、引用锚点、标题结构、HTML/PDF 产物完整性。

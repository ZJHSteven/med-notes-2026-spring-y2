...
## 当前未完成计划
- `循环系统/PBL作业/Case2` 整幕思维导图收口：
  1. 对照 `血液与肿瘤/PBL作业/Case1-第三幕/CML-mindmap.html` 的 Markmap 结构和视觉样式，确定 Case2 导图也采用“右上角三幕证据链 + 右下角图例 + 主干六大模块”的形式。
  2. 从 `Case2-第一幕.md`、`Case2-第二幕.md`、`Case2-第三幕.md` 提炼刘阿姨急性前壁 STEMI 的完整病例链，不改动原三幕正文。
  3. 新增 `循环系统/PBL作业/Case2-第三幕/AMI-mindmap.html`，主干覆盖危险因素/一级预防、发病机制、诊断与分层、鉴别诊断、治疗与二级预防、预后与复发风险。
  4. 使用浏览器实际打开本地 HTML，检查 Markmap 是否渲染、核心节点是否可见、悬浮证据卡和图例是否正常显示。
  5. 更新 `PROGRESS.md`，记录本次导图产物、验证方式和后续可选扩展。

...
## 当前未完成计划
- `循环系统/PBL作业/Case2-第二幕` 二次修订与教材核对
  - [ ] 先把根目录 `内科学 （第10版）.pdf` 加入 `.gitignore`，避免误入 Git 历史。
  - [ ] 修复 `循环系统/PBL作业/Case2-第二幕/build_html.py` 对 Markdown 表格不渲染的问题，并补充样式。
  - [ ] 用根目录《内科学》第10版 PDF 逐条核对第二幕中所有教材/专著出处页码，去掉 `【待核对教材页码】` 和未核准表述。
  - [ ] 重新生成第二幕 HTML/PDF，并验证表格、引用跳转、教材页码、残留占位词和 PDF 完整性。

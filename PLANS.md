...
## 当前未完成计划
- 生成 `消化系统/PBL作业/Case3-第二幕` 的主持人最终整合版 PPT，按既定汇报顺序把 9 位同学的题目页/文献页与张家赫主持页合成一份可直接上课播放的终稿。
  - 先核对真实素材：当前目录下已确认存在 `张家赫.pptx` 主持人底稿、9 位同学分目录素材，以及 `新建 文本文档.txt` 中写明的“临床问题排序 + 文献机制排序 + 串场逻辑”。
  - 先建立题号映射，避免串错：当前已初步对应为 `Q6=刘子硕`、`Q1=张家赫`、`Q2=张志擎`、`Q4=常瑞琪`、`Q3=刘骐玮`、`Q5=谢尚锦`、`Q7=蒋玉梅`、`Q8=姚丁睿`、`Q9=林诚亚`、`Q10=王鹤`；文献部分按“总框架 -> 病因/炎症/免疫迁移 -> LSEC/EMT/自噬/氧化应激 -> HSC综述补充”顺序整合。
  - 默认保留同学原稿页，主持人页负责总起、过渡和总结，不强行统一重做每位同学页面。
  - 需要排除明显错文件：`王鹤/case2第一幕.pptx` 是上一案例旧素材，不应混入 `Case3-第二幕` 终稿。
  - 终稿验收标准：`PPT 可正常打开`、`页序与主持稿一致`、`无空白/错插/重复旧案页`、`能导出整套预览图`、`文件写回到 Case3-第二幕目录`。

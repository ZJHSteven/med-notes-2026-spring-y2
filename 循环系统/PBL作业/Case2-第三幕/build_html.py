"""
这个脚本的作用：
1. 将当前目录中的第三幕课程作业 Markdown 转成带样式的 HTML。
2. 兼容两种链接写法：
   - 新写法：`[显示文字](链接 "悬浮标题")`
   - 旧写法：`[显示文字][引用名]` / `[引用名]`
3. 保留“问题区前的导语段落”，不再像旧脚本那样直接吞掉。
4. 将正文中的 `[1]`、`[1,2]` 这类数字引用渲染成上标，并跳转到本题出处列表。
5. 将 Markdown 的块引用 `>` 渲染成黄色提示框，和总结区保持同一视觉语言。
6. 优先通过 `uv run --with playwright` 调用浏览器内核导出 A4 PDF；
   若本机没有 `uv`，再退回 Edge 命令行打印作为兜底。

使用方式：
1. 在第三幕目录内直接运行：`python build_html.py`
2. 或者显式指定输入文件：`python build_html.py "Case2-第三幕.md"`
...
# 这个正则只识别“纯数字引用”，例如 `[1]`、`[1,2]`、`[1-3]`。
# 它不会处理普通 Markdown 链接，因为链接会在后续分支里单独解析。
...
def build_citation_anchor(citation_text: str, citation_scope: str | None) -> str:
    """把正文里的数字引用渲染成可跳转的上标引用。

    参数：
    - citation_text：方括号里的原始数字部分，例如 `1`、`1,2`、`1-3`。
    - citation_scope：当前问题的引用命名空间，例如 `q1`；为空时只按普通文本输出。

    输出：
    - 一个 `<sup>` HTML 片段；每个数字会链接到本题 `④出处` 中对应的 `<li>`。

    设计取舍：
    - 每一题的出处编号都会从 1 重新开始，所以不能做全局 `#ref-1`。
    - 这里使用 `q1-ref-1` 这样的锚点，避免不同问题之间编号冲突。
    """

    # 没有题目作用域时，说明当前块不属于某个问题，保守地按普通文本渲染。
    if not citation_scope:
        return format_plain_text(f"[{citation_text}]")

    rendered_parts: list[str] = ["["]

    # 保留逗号、中文逗号、短横线等分隔符的原貌，同时只给数字部分加跳转。
    for token in re.split(r"(\s*(?:,|，|-|–|—)\s*)", citation_text):
        if not token:
            continue

        number_text = token.strip()
        if number_text.isdigit():
            safe_number = html.escape(number_text)
            href = f"#{citation_scope}-ref-{safe_number}"
            rendered_parts.append(f'<a href="{href}">{safe_number}</a>')
        else:
...
def render_inline(text: str, refs: dict[str, ReferenceLink], citation_scope: str | None = None) -> str:
    ...
    while index < len(text):
        ...
        # 情况零：正文数字引用，例如 `[1]` 或 `[1,2]`。
        # 这里要求后面不能紧跟 `(` 或 `[`，否则它应当优先按 Markdown 链接处理。
        if NUMERIC_CITATION_RE.fullmatch(display_text.strip()) and not (
            next_index < len(text) and text[next_index] in "([" 
        ):
            flush_plain_buffer()
            result.append(build_citation_anchor(display_text.strip(), citation_scope))
...
def render_block_content(
    ...
    参数：
    - citation_scope：当前正文属于哪一题；用于把 `[1]` 渲染成跳转到本题出处的上标。
    - reference_scope：当前块是否是出处列表；若是，则给有序列表条目生成稳定 `id`。
    """
    ...
    while index < len(lines):
        ...
        # 四到六级标题通常出现在“学习内容”内部，用作问题内部的小层次。
        heading_match = re.match(r"^#{4,6}\s+(.*\S)\s*$", raw_line)
        if heading_match:
            html_blocks.append(
                f'<h4 class="mini-title">{render_inline(heading_match.group(1).strip(), refs, citation_scope)}</h4>'
            )
            index += 1
            continue
        ...
        if stripped_line.startswith(">"):
            ...
            while index < len(lines) and lines[index].strip().startswith(">"):
                ...
                index += 1
            inner_html = render_block_content("\n".join(quote_lines), refs, citation_scope, reference_scope)
        ...
        if ordered_match:
            ...
            while index < len(lines):
                ...
                if not item_match:
                    break
                item_number = item_match.group(0).split(".", 1)[0]
                id_attr = f' id="{reference_scope}-ref-{html.escape(item_number)}"' if reference_scope else ""
                items.append(
                    f"<li{id_attr}>{render_inline(item_match.group(1).strip(), refs, citation_scope)}</li>"
                )
                index += 1
        ...
        if unordered_match:
            ...
            while index < len(lines):
                ...
                if not item_match:
                    break
                items.append(f"<li>{render_inline(item_match.group(1).strip(), refs, citation_scope)}</li>")
                index += 1
        ...
        html_blocks.append(f"<p>{render_inline(paragraph_text, refs, citation_scope)}</p>")
...
def render_document(document: DocumentModel, css_name: str = "style.css") -> str:
    ...
    # 依次渲染每个问题块。
    for question_index, question in enumerate(document.questions, start=1):
        question_scope = f"q{question_index}"
        ...
        if question.intro.strip():
            ...
            parts.append(
                render_block_content(
                    trim_outer_horizontal_rules(question.intro),
                    document.refs,
                    citation_scope=question_scope,
                )
            )
            parts.append("            </div>")

        for sub_section in question.sub_sections:
            is_reference_section = sub_section.title.startswith("④出处")
            parts.append('            <div class="sub-section">')
            parts.append(f'                <h3 class="sub-title">{render_inline(sub_section.title, document.refs)}</h3>')
            parts.append(
                render_block_content(
                    trim_outer_horizontal_rules(sub_section.content),
                    document.refs,
                    citation_scope=None if is_reference_section else question_scope,
                    reference_scope=question_scope if is_reference_section else None,
                )
            )
            parts.append("            </div>")
